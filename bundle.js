	  ...
	  */
	  ...
	  moment.fn.toJD = function (dateObj) {
	    ...
	    */
	    ...
	    if (year === 1582 && dateObj.month() === 9 && dateObj.date() <= 14 && dateObj.date() >= 5) {
	      ...
	      var currDecYear = dateObj.year() + (dateObj.month() + 1) / 12 + dateObj.date() / dateObj.daysInMonth();
	      ...
	      if (currDecYear < julCalDecYear) {
	        ...
	        var jdn = dateObj.date() + Math.floor((153 * m + 2) / 5) + 365 * y + Math.floor(y / 4) - 32083;
	        var jd = jdn +
	        // validate UTC (JD is always in UTC)
	        ((dateObj._isUTC ? dateObj.hour() : dateObj.hour() - dateObj.utcOffset() / 60) - 12) / 24 + dateObj.minute() / 1440 + dateObj.second() / 86400;
	      } else {
	        // Gregorian calendar
	        var a = Math.floor((14 - (dateObj.month() + 1)) / 12);
	        var y = year + 4800 - a;
	        var m = dateObj.month() + 1 + 12 * a - 3;
	        var jdn = dateObj.date() + Math.floor((153 * m + 2) / 5) + 365 * y + Math.floor(y / 4) - Math.floor(y / 100) + Math.floor(y / 400) - 32045;
	        var jd = jdn +
	        // validate UTC (JD is always in UTC)
	        ((dateObj._isUTC ? dateObj.hour() : dateObj.hour() - dateObj.utcOffset() / 60) - 12) / 24 + dateObj.minute() / 1440 + (dateObj.second() + dateObj.millisecond() / 1000) / 86400;
	      }
	    }
	    return jd;
	  };
	  ...
	  moment.fn.fromJD = function (myJD) {
	    ...
	    */
	    ...
	    // the returned value complies with ISO-8601:
	    // e.g. myYear = -9 (result from the algorithm above)
	    ...
	    */
	    return moment.utc().year(myYear).month(myMonth - 1).date(myDay).hour(myHour).minute(myMinute).second(mySecond).millisecond(myMillisecond);
	  };
